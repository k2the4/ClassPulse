...
export interface AttendanceSheetStudent { enrollmentNo: string; present: boolean; }
...
function cleanEnrollment(value: unknown): string {
  ...
  if (scientific) { const number = Number(raw); if (Number.isFinite(number)) return Math.round(number).toString(); }
  return raw.replace(/\.0$/, "");
}
function columnName(index: number): string { let n = index + 1, result = ""; while (n > 0) { const remainder = (n - 1) % 26; result = String.fromCharCode(65 + remainder) + result; n = Math.floor((n - 1) / 26); } return result; }
function findStudentHeader(rows: string[][]): number { return rows.findIndex((row) => normalize(row?.[0]) === "s.no" && normalize(row?.[1]).includes("enrollment") && normalize(row?.[2]).includes("student")); }
function sessionHeaderMatches(value: unknown, date: string, slot: string): boolean { const text = normalize(value); return text === normalize(`${date} | ${slot}`) || text === normalize(`${date} ${slot}`); }
function sessionSortKey(value: unknown): string | null { const text = String(value ?? "").trim(); const match = text.match(/^(\d{4}-\d{2}-\d{2})\s*\|\s*(.+)$/); if (!match) return null; const slotIndex = TIME_SLOT_ORDER.findIndex((item) => normalize(item) === normalize(match[2])); return `${match[1]}|${String(slotIndex === -1 ? 999 : slotIndex).padStart(3, "0")}|${normalize(match[2])}`; }

async function refreshLatestSessionMetadata(params: { sheets: ReturnType<typeof getSheetsClient>; spreadsheetId: string; title: string; teacherName: string; }): Promise<void> {
  // Keep the diary header fixed. Session IDs and subject/teacher labels must
  // not live in the attendance columns because inserting a past session would
  ...
  await params.sheets.spreadsheets.values.batchUpdate({
    ...
    requestBody: {
      ...
      data: [
        { range: `'${params.title}'!B2`, values: [[params.teacherName]] },
        { range: `'${params.title}'!C3:AZ3`, values: [new Array(50).fill("")] },
        { range: `'${params.title}'!A4:AZ4`, values: [new Array(52).fill("")] },
      ],
    },
  });
}
...
export async function writeTeacherDiaryAttendanceSafe(params: {
  ...
  for (const [enrollmentNo, rowIndex] of enrollmentRows.entries()) { const rowNumber = rowIndex + 1; writeRanges.push({ range: `'${title}'!${startCol}${rowNumber}`, values: [[1]] }); writeRanges.push({ range: `'${title}'!${endCol}${rowNumber}`, values: [[incoming.get(enrollmentNo) ? 1 : 0]] }); }
  ...
  await refreshLatestSessionMetadata({ sheets, spreadsheetId: params.spreadsheetId, title, teacherName: params.teacherName });
