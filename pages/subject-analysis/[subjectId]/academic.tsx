...
export default function SubjectAcademicPage() {
  ...
  const statsFor = (exam: "midsem1" | "midsem2") => {
    const rows = students.map((s: any, index: number) => ({ sno: index + 1, enrollmentNo: s.enrollmentNo, name: s.name, marks: valueFor(s, exam) }));
  ...
  return <div className="analysis-layout">
    ...
    <main className="analysis-page">
      ...
      {data && (view === "midsem1" || view === "midsem2") && <>
        ...
        <section className="analysis-content-grid academic-content-grid"><section className="analysis-panel analysis-table-panel"><div className="analysis-panel-head"><div><h3>Data Sheet</h3><p style={{marginTop:4,color:"#98a2b3",fontSize:11}}>Pass mark: 12/30 (40%){selectedTier?` · Filtered: ${selectedTier}`:""}</p></div><div style={{display:"flex",alignItems:"center",gap:8}}>{selectedTier&&<button className="analysis-secondary" style={{padding:"5px 8px"}} onClick={()=>setSelectedTier(null)}><X size={13}/>Clear</button>}<span className="analysis-count">{displayedRows.length} Students</span></div></div>
          <div className="analysis-table-wrap academic-table-wrap"><table className="analysis-table academic-student-table"><colgroup><col className="academic-sno-col"/><col className="academic-enrollment-col"/><col className="academic-name-col"/><col className="academic-marks-col"/><col className="academic-percent-col"/><col className="academic-status-col"/></colgroup><thead><tr><th>S.No.</th><th>Enrollment No.</th><th>Student</th><th>Marks</th><th>Percentage</th><th>Status</th></tr></thead><tbody>{displayedRows.map(row=>{const pass=row.marks>=12,p=Math.round(row.marks/MAX*100);return <tr key={row.enrollmentNo}><td>{row.sno}</td><td className="academic-enrollment-cell">{row.enrollmentNo}</td><td><span className="student-cell"><span className="student-avatar">{initials(row.name)}</span>{row.name}</span></td><td className={pass?"change-up":"change-down"}>{row.marks}</td><td>{p}%</td><td><span className={`trend-badge ${pass?"trend-up":"trend-down"}`}>{pass?"Pass":"Fail"}</span></td></tr>;})}</tbody></table></div>
    ...
    <style jsx global>{`
      ...
      .academic-enrollment-col { width: 18%; }
      .academic-name-col { width: 29%; }
      .academic-marks-col { width: 13%; }
      .academic-percent-col { width: 13%; }
      .academic-status-col { width: 20%; }
      ...
      .academic-student-table .student-cell { justify-content: flex-start; }
