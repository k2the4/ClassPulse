...
export default function SubjectAcademicPage() {
  ...
  const combinedRows = useMemo(() => students.map((s: any) => ({ enrollmentNo: s.enrollmentNo, name: s.name, first: valueFor(s, "midsem1"), second: valueFor(s, "midsem2"), combined: valueFor(s, "combined"), max: valueFor(s, "max") })), [students]);
  ...
  const getCombinedSortValue = (row: typeof combinedRows[number]) => combinedSort === "midsem1" ? row.first : combinedSort === "midsem2" ? row.second : combinedSort === "max" ? row.max : row.combined;
  const sortedCombinedRows = useMemo(() => {
    if (sortOrder === "none") return [...filteredCombinedRows];
    return [...filteredCombinedRows].sort((a, b) => { const difference = getCombinedSortValue(a) - getCombinedSortValue(b); return difference !== 0 ? (sortOrder === "asc" ? difference : -difference) : a.name.localeCompare(b.name); });
  ...
  return <div className="analysis-layout">
    <aside className="analysis-sidebar">
      ...
      <nav className="analysis-side-nav"><a href="/dashboard"><LayoutDashboard size={18} />Dashboard</a><a href="/classes"><BookOpen size={18} />Class Analysis</a><a className="is-active" href={typeof subjectId === "string" ? `/subject-analysis/${subjectId}/academic` : "#"}><GraduationCap size={18} />Subject Analysis</a></nav>
      <RawDataButton sheetId={sheetId} /><div className="analysis-side-footer">ClassPulse Teacher Portal</div>
    </aside>
    <main className="analysis-page">
      ...
      <div className="analysis-view-switch">{([["midsem1", "Midsem 1"], ["midsem2", "Midsem 2"], ["combined", "Combined"], ["summary", "Summary"]] as [AcademicView, string][]).map(([k, l]) => <button key={k} className={view === k ? "is-active" : ""} onClick={() => setView(k)}>{l}</button>)}</div>
      ...
      {data && view === "combined" && <>
        ...
        <section className="combined-layout">
          <section className="analysis-panel analysis-table-panel combined-table-panel">
            <div className="analysis-panel-head combined-table-head"><div><h3>All Students</h3><p style={{ marginTop: 4, color: "#98a2b3", fontSize: 11 }}>{selectedTier ? `Filtered: ${selectedTier}` : "Combined Midsem results"}</p></div><div className="combined-table-actions"><div className="combined-control"><label htmlFor="combined-sort-field">Grade / Sort By</label><select id="combined-sort-field" value={combinedSort} onChange={(e) => setCombinedSort(e.target.value as CombinedSort)}><option value="combined">Combined (average)</option><option value="midsem1">Midsem 1</option><option value="midsem2">Midsem 2</option><option value="max">Maximum score</option></select></div><div className="combined-control"><label htmlFor="combined-sort-order">Order</label><select id="combined-sort-order" value={sortOrder} onChange={(e) => setSortOrder(e.target.value as SortOrder)}><option value="desc">High to Low</option><option value="asc">Low to High</option><option value="none">No Sort</option></select></div>{selectedTier && <button className="analysis-secondary combined-clear" style={{ padding: "5px 8px" }} onClick={() => setSelectedTier(null)}><X size={13} />Clear</button>}<span className="analysis-count">{sortedCombinedRows.length} Students</span></div></div>
            <div className="analysis-table-wrap combined-table-wrap"><table className="analysis-table combined-student-table"><colgroup><col className="combined-sno-col" /><col className="combined-name-col" /><col className="combined-enrollment-col" /><col className="combined-mark-col" /><col className="combined-mark-col" /><col className="combined-mark-col" /><col className="combined-mark-col" /><col className="combined-grade-col" /></colgroup><thead><tr><th>{sortOrder === "none" ? "S.No." : "Rank"}</th><th>Student</th><th>Enrollment No.</th><th>Midsem 1</th><th>Midsem 2</th><th>Combined</th><th>Max</th><th>Grade</th></tr></thead><tbody>{sortedCombinedRows.map((row, index) => { const tier = tierFor(getCombinedSortValue(row)); const rank = index + 1; const serial = combinedRows.findIndex(r => r.enrollmentNo === row.enrollmentNo) + 1; const sortedColumnClass = (field: CombinedSort) => sortOrder !== "none" && combinedSort === field ? `tier-mark ${gradeClass(tier)}` : ""; return <tr key={row.enrollmentNo}><td>{sortOrder === "none" ? serial : rank}</td><td className="combined-name-cell">{row.name}</td><td>{row.enrollmentNo}</td><td className={sortedColumnClass("midsem1")}>{row.first}</td><td className={sortedColumnClass("midsem2")}>{row.second}</td><td className={sortedColumnClass("combined")}><strong>{row.combined}</strong></td><td className={sortedColumnClass("max")}>{row.max}</td><td><button type="button" className={`analysis-grade-badge ${gradeClass(tierFor(row.combined))}`} onClick={() => setSelectedTier(selectedTier === tierFor(row.combined) ? null : tierFor(row.combined))}>{tierFor(row.combined)}</button></td></tr>; })}{!sortedCombinedRows.length && <tr><td colSpan={8} style={{ textAlign: "center", padding: 32, color: "#667085" }}>No students match the selected filter.</td></tr>}</tbody></table></div>
          </section>
      ...
      {data && view === "summary" && <>
        ...
        <section className="summary-metric-grid"><Metric label="Overall Class Average" value={summaryAverage} detail="combined average" /><Metric label="Highest Combined Score" value={combinedHighest} detail={combinedHighestNames[0] || "—"} /><Metric label="Overall Pass Rate" value={`${summaryPassRate}%`} detail="students scoring 12 or more" /><Metric label="Midsem 1 Average" value={midsem1.average} detail="out of 30 marks" /><Metric label="Midsem 2 Average" value={midsem2.average} detail="out of 30 marks" /></section>
    ...
    <style jsx global>{`
      .combined-layout { display: grid; grid-template-columns: minmax(0, 1.7fr) minmax(300px, .9fr); gap: 16px; align-items: start; width: 100%; min-width: 0; }
      .combined-table-panel { min-width: 0; overflow: hidden; }
      .combined-table-head { align-items: flex-end; gap: 14px; }
      .combined-table-actions { display: flex; align-items: flex-end; justify-content: flex-end; gap: 8px; min-width: 0; }
      .combined-control { min-width: 145px; }
      .combined-control label { display: block; margin-bottom: 6px; color: #667085; font-size: 10px; font-weight: 600; }
      .combined-control select { width: 100%; height: 36px; border: 1px solid #d8e0ea; border-radius: 8px; background: #fff; color: #344054; padding: 0 9px; font-size: 12px; outline: none; box-sizing: border-box; }
      .combined-control select:focus { border-color: #4b2e91; box-shadow: 0 0 0 2px rgba(75,46,145,.12); }
      .combined-clear { flex: 0 0 auto; height: 36px; }
      .combined-table-wrap { min-height: 560px; max-height: 560px; width: 100%; max-width: 100%; overflow-x: hidden !important; overflow-y: auto; }
      .combined-student-table { width: 100% !important; min-width: 0 !important; max-width: 100%; table-layout: fixed; }
      .combined-student-table th, .combined-student-table td { min-width: 0 !important; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
      .combined-student-table .combined-sno-col { width: 6%; }
      .combined-student-table .combined-name-col { width: 23%; }
      .combined-student-table .combined-enrollment-col { width: 17%; }
      .combined-student-table .combined-mark-col { width: 9.5%; }
      .combined-student-table .combined-grade-col { width: 15%; }
      .combined-student-table th:not(:nth-child(2)), .combined-student-table td:not(:nth-child(2)) { text-align: center; }
      .combined-student-table .combined-name-cell { text-align: left !important; }
      .combined-student-table .tier-mark { font-weight: 700; }
      .combined-student-table .tier-mark.excellent { color: #4d75d0 !important; }
      .combined-student-table .tier-mark.good { color: #15966a !important; }
      .combined-student-table .tier-mark.attention { color: #f59e0b !important; }
      .combined-student-table .tier-mark.risk { color: #ef4444 !important; }
      .combined-right-column { display: grid; gap: 16px; min-width: 0; }
      ...
      .combined-chart { height: 280px; margin-top: 10px; }
      ...
      @media (max-width: 800px) { .combined-table-head { align-items: stretch; flex-direction: column; } .combined-table-actions { justify-content: flex-start; flex-wrap: wrap; } .combined-layout, .summary-main-grid { grid-template-columns: 1fr; } .combined-right-column, .summary-metric-grid, .summary-filter-grid { grid-template-columns: 1fr; } .combined-control { min-width: 0; flex: 1; } .combined-table-wrap, .summary-table-wrap { min-height: 480px; max-height: 480px; } }
      @media (max-width: 600px) { .combined-table-actions { flex-direction: column; align-items: stretch; } .combined-control { width: 100%; } .summary-filter-title { flex-direction: column; } }
    `}</style>
  </div>;
}
