...
export default function SubjectAcademicPage() {
  ...
  const statsFor = (exam: "midsem1" | "midsem2") => {
    const baseRows = students.map((s: any, index: number) => ({ sno: index + 1, enrollmentNo: s.enrollmentNo, name: s.name, marks: valueFor(s, exam) }));
    const rankByEnrollment = new Map<string, number>();
    [...baseRows].sort((a, b) => b.marks - a.marks || a.name.localeCompare(b.name)).forEach((row, index, sorted) => {
      const previous = index > 0 ? sorted[index - 1].marks : null;
      rankByEnrollment.set(row.enrollmentNo, previous === row.marks ? (rankByEnrollment.get(sorted[index - 1].enrollmentNo) || index + 1) : index + 1);
    });
    ...
    return { rows, average: marks.length ? round1(marks.reduce((a,b) => a+b, 0) / marks.length) : 0, median: median(marks), highest, highestNames: rows.filter(r => r.marks === highest).map(r => r.name), passRate: marks.length ? Math.round(rows.filter(r => r.marks >= 12).length / marks.length * 100) : 0, counts, sorted: [...rows].sort((a,b) => b.marks-a.marks) };
  };

  const midsem1 = useMemo(() => statsFor("midsem1"), [students]);
  const midsem2 = useMemo(() => statsFor("midsem2"), [students]);
  const combinedRows = useMemo(() => students.map((s:any, index:number) => ({ sno:index+1, enrollmentNo:s.enrollmentNo, name:s.name, first:valueFor(s,"midsem1"), second:valueFor(s,"midsem2"), combined:valueFor(s,"combined"), max:valueFor(s,"max") })), [students]);
  const combinedCounts = useMemo(() => { const r=TIERS.reduce((a,t)=>({...a,[t]:0}),{} as Record<Tier,number>); combinedRows.forEach(row=>r[tierFor(row.combined)]++); return r; }, [combinedRows]);
  ...
  const filteredCombinedRows = useMemo(() => selectedTier ? combinedRows.filter(row => tierFor(row.combined) === selectedTier) : combinedRows, [combinedRows, selectedTier]);
  const sortedCombinedRows = useMemo(() => {
    if (sortOrder === "none") return [...filteredCombinedRows];
    const getValue = (row:any) => combinedSort === "midsem1" ? row.first : combinedSort === "midsem2" ? row.second : combinedSort === "max" ? row.max : row.combined;
    return [...filteredCombinedRows].sort((a,b) => { const diff=getValue(a)-getValue(b); return diff ? (sortOrder === "asc" ? diff : -diff) : a.name.localeCompare(b.name); });
  ...
  const combinedAverage = combinedRows.length ? round1(combinedRows.reduce((s,r)=>s+r.combined,0)/combinedRows.length) : 0;
  const combinedHighest = combinedRows.length ? Math.max(...combinedRows.map(r=>r.combined)) : 0;
  const combinedHighestNames = combinedRows.filter(r=>r.combined===combinedHighest).map(r=>r.name);
  const summaryRows = useMemo(() => combinedRows.map(row => { const marks=summaryExam === "midsem1" ? row.first : summaryExam === "midsem2" ? row.second : summaryExam === "max" ? row.max : row.combined; return {...row,marks,tier:tierFor(marks),change:round1(row.second-row.first)}; }).filter(row => { const lower=Number(summaryLower), upper=Number(summaryUpper); return row.marks >= (Number.isFinite(lower)?lower:0) && row.marks <= (Number.isFinite(upper)?upper:MAX) && (summaryTier === "all" || row.tier === summaryTier); }).sort((a,b)=>summarySort === "none" ? 0 : summarySort === "desc" ? b.marks-a.marks : a.marks-b.marks), [combinedRows,summaryExam,summaryTier,summaryLower,summaryUpper,summarySort]);
  const summaryAverage = combinedRows.length ? round1(combinedRows.reduce((s,r)=>s+r.combined,0)/combinedRows.length) : 0;
  const summaryPassRate = combinedRows.length ? Math.round(combinedRows.filter(r=>r.combined>=12).length/combinedRows.length*100) : 0;
  const increases = [...combinedRows].map(r=>({...r,change:round1(r.second-r.first)})).filter(r=>r.change>0).sort((a,b)=>b.change-a.change).slice(0,5);
  const decreases = [...combinedRows].map(r=>({...r,change:round1(r.second-r.first)})).filter(r=>r.change<0).sort((a,b)=>a.change-b.change).slice(0,5);
  const applyCombinedControls=()=>{setCombinedSort(combinedSortDraft);setSortOrder(sortOrderDraft);};
  const resetSummaryControls=()=>{setSummaryExam("combined");setSummaryTier("all");setSummaryLower("0");setSummaryUpper("30");setSummarySort("none");setSummaryExamDraft("combined");setSummaryTierDraft("all");setSummaryLowerDraft("0");setSummaryUpperDraft("30");setSummarySortDraft("none");};
  const applySummaryControls=()=>{setSummaryExam(summaryExamDraft);setSummaryTier(summaryTierDraft);setSummaryLower(summaryLowerDraft);setSummaryUpper(summaryUpperDraft);setSummarySort(summarySortDraft);};
  ...
  return <div className="analysis-layout">
    ...
    <main className="analysis-page">
      ...
      {typeof subjectId === "string" && <SubjectAnalysisNav subjectId={subjectId}/>}<div className="analysis-view-switch">{([["midsem1","Midsem 1"],["midsem2","Midsem 2"],["combined","Combined"],["summary","Summary"]] as [AcademicView,string][]).map(([k,l])=><button key={k} className={view===k?"is-active":""} onClick={()=>setView(k)}>{l}</button>)}</div>
      {error && <div className="analysis-panel" style={{padding:14,marginBottom:16,color:"#b42318"}}>{error}</div>}{loading&&!data&&<div style={{padding:40,color:"#667085",fontSize:13}}>Loading academic analysis...</div>}
      {data && (view === "midsem1" || view === "midsem2") && <>
        <section className="analysis-hero"><div className="analysis-hero-copy"><h2>{activeLabel}</h2><p>Raw Midsem marks out of 30, class statistics, and performance tiers.</p></div><Metric label="Class Average" value={activeStats.average} detail="out of 30 marks"/><Metric label="Class Median" value={activeStats.median} detail="middle class score"/><Metric label="Pass Rate" value={`${activeStats.passRate}%`} detail="students scoring 12 or more"/></section>
        <section className="analysis-content-grid academic-content-grid"><section className="analysis-panel analysis-table-panel"><div className="analysis-panel-head"><div><h3>Data Sheet</h3><p style={{marginTop:4,color:"#98a2b3",fontSize:11}}>Pass mark: 12/30 (40%){selectedTier?` · Filtered: ${selectedTier}`:""}</p></div><div style={{display:"flex",alignItems:"center",gap:8}}>{selectedTier&&<button className="analysis-secondary" style={{padding:"5px 8px"}} onClick={()=>setSelectedTier(null)}><X size={13}/>Clear</button>}<span className="analysis-count">{displayedRows.length} Students</span></div></div>
          <div className="analysis-table-wrap academic-table-wrap"><table className="analysis-table academic-student-table"><colgroup><col className="academic-sno-col"/><col className="academic-enrollment-col"/><col className="academic-name-col"/><col className="academic-marks-col"/><col className="academic-status-col"/><col className="academic-rank-col"/></colgroup><thead><tr><th>S.No.</th><th>Enrollment No.</th><th>Student</th><th>Marks</th><th>Status</th><th>Rank</th></tr></thead><tbody>{displayedRows.map(row=>{const pass=row.marks>=12;return <tr key={row.enrollmentNo}><td>{row.sno}</td><td className="academic-enrollment-cell">{row.enrollmentNo}</td><td>{row.name}</td><td className={pass?"change-up":"change-down"}>{row.marks}</td><td><span className={`trend-badge ${pass?"trend-up":"trend-down"}`}>{pass?"Pass":"Fail"}</span></td><td>{row.rank}</td></tr>;})}</tbody></table></div>
        </section><div className="analysis-right-stack"><section className="analysis-panel academic-stats-panel"><div className="academic-highest"><span>Highest Score</span><strong>{activeStats.highest}</strong><p>{activeStats.highestNames.join(", ")||"—"}</p></div><div className="academic-tier-grid">{TIERS.map(t=><button key={t} type="button" className="academic-tier-card" onClick={()=>setSelectedTier(selectedTier===t?null:t)} style={{borderColor:`${COLORS[t]}55`,cursor:"pointer",boxShadow:selectedTier===t?`0 0 0 2px ${COLORS[t]}33`:undefined}}><span>{t}</span><strong style={{color:COLORS[t]}}>{activeStats.counts[t]}</strong></button>)}</div></section><section className="analysis-panel analysis-chart-panel"><h3>Performance Tier</h3><p>Click a chart segment to filter the student table.</p><div className="analysis-chart"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={activePie} dataKey="value" nameKey="name" innerRadius={54} outerRadius={82} paddingAngle={3} onClick={(entry:any)=>{const t=entry?.name as Tier|undefined;if(t&&TIERS.includes(t))setSelectedTier(selectedTier===t?null:t);}}>{activePie.map(e=><Cell key={e.name} fill={e.color} cursor="pointer" opacity={!selectedTier||selectedTier===e.name?1:.35}/>)}</Pie><Tooltip/></PieChart></ResponsiveContainer></div></section><div className="academic-rank-grid"><section className="analysis-panel academic-rank-panel"><h3>Top 5 Highest Scorers</h3>{activeStats.sorted.slice(0,5).map((r,i)=><div className="academic-rank-row" key={r.enrollmentNo}><span>{i+1}</span><p>{r.name}</p><strong>{r.marks}</strong></div>)}</section><section className="analysis-panel academic-rank-panel"><h3>Bottom 5 At-Risk Students</h3>{[...activeStats.sorted].reverse().slice(0,5).map((r,i)=><div className="academic-rank-row" key={r.enrollmentNo}><span>{i+1}</span><p>{r.name}</p><strong className="is-negative">{r.marks}</strong></div>)}</section></div></div></section>
      </>}
      {data && view === "combined" && <><section className="analysis-hero combined-hero"><div className="analysis-hero-copy"><h2>Midsem Combined</h2><p>Compare both Midsem examinations in one view.</p></div><Metric label="Students" value={combinedRows.length} detail="combined records"/><Metric label="Class Average" value={combinedAverage} detail="combined average"/><Metric label="Best Score" value={combinedHighest} detail={combinedHighestNames.join(", ")||"best individual result"}/></section><section className="combined-layout"><section className="analysis-panel analysis-table-panel combined-table-panel"><div className="combined-table-toolbar"><div className="analysis-panel-head"><div><h3>All Students</h3><p style={{marginTop:4,color:"#98a2b3",fontSize:11}}>{selectedTier?`Filtered: ${selectedTier}`:"Combined Midsem results"}</p></div><div style={{display:"flex",alignItems:"center",gap:8}}>{selectedTier&&<button className="analysis-secondary" style={{padding:"5px 8px"}} onClick={()=>setSelectedTier(null)}><X size={13}/>Clear</button>}<span className="analysis-count">{sortedCombinedRows.length} Students</span></div></div><div className="combined-inline-controls"><label><span>Grade / Sort By</span><select value={combinedSortDraft} onChange={e=>setCombinedSortDraft(e.target.value as CombinedSort)}><option value="combined">Combined (average)</option><option value="midsem1">Midsem 1</option><option value="midsem2">Midsem 2</option><option value="max">Maximum score</option></select></label><label><span>Order</span><select value={sortOrderDraft} onChange={e=>setSortOrderDraft(e.target.value as SortOrder)}><option value="desc">High to Low</option><option value="asc">Low to High</option><option value="none">No Sort</option></select></label><button className="analysis-primary combined-apply-button" onClick={applyCombinedControls}>Apply</button></div></div><div className="analysis-table-wrap combined-table-wrap"><table className="analysis-table"><thead><tr><th>Rank</th><th>Student</th><th>Midsem 1</th><th>Midsem 2</th><th>Combined</th><th>Max</th><th>Grade</th></tr></thead><tbody>{sortedCombinedRows.map((row,index)=>{const tier=tierFor(row.combined);return <tr key={row.enrollmentNo}><td>{sortOrder==="none"?"":index+1}</td><td><span className="student-cell"><span className="student-avatar">{initials(row.name)}</span>{row.name}</span></td><td>{row.first}</td><td>{row.second}</td><td><strong>{row.combined}</strong></td><td>{row.max}</td><td><button type="button" className={`analysis-grade-badge ${gradeClass(tier)}`} onClick={()=>setSelectedTier(selectedTier===tier?null:tier)}>{tier}</button></td></tr>})}{!sortedCombinedRows.length&&<tr><td colSpan={7} style={{textAlign:"center",padding:32,color:"#667085"}}>No students match the selected filter.</td></tr>}</tbody></table></div></section><div className="combined-right-column"><section className="analysis-panel combined-chart-panel"><div className="combined-panel-title"><div><h3>Grade Distribution</h3><p>Click a bar to filter the table by performance tier.</p></div><span><strong>{combinedRows.length}</strong> Students</span></div><div className="combined-chart"><ResponsiveContainer width="100%" height="100%"><BarChart data={TIERS.map(t=>({name:t,count:combinedCounts[t]}))} onClick={(state:any)=>{const tier=state?.activeLabel as Tier|undefined;if(tier&&TIERS.includes(tier))setSelectedTier(selectedTier===tier?null:tier);}}><CartesianGrid strokeDasharray="3 3" vertical={false}/><XAxis dataKey="name" fontSize={10}/><YAxis allowDecimals={false} fontSize={11}/><Tooltip/><Bar dataKey="count" radius={[6,6,0,0]}>{TIERS.map(t=><Cell key={t} fill={COLORS[t]} cursor="pointer" opacity={!selectedTier||selectedTier===t?1:.35}/>)}</Bar></BarChart></ResponsiveContainer></div></section><section className="analysis-panel combined-tier-panel"><h3>Performance Tiers</h3><div className="academic-tier-grid">{TIERS.map(t=><button key={t} type="button" className="academic-tier-card" onClick={()=>setSelectedTier(selectedTier===t?null:t)} style={{borderColor:`${COLORS[t]}55`,cursor:"pointer",boxShadow:selectedTier===t?`0 0 0 2px ${COLORS[t]}33`:undefined}}><span>{t}</span><strong style={{color:COLORS[t]}}>{combinedCounts[t]}</strong></button>)}</div></section><div className="academic-rank-grid"><section className="analysis-panel academic-rank-panel"><h3>Top 5 Highest Scorers</h3>{combinedRankRows.slice(0,5).map((r,i)=><div className="academic-rank-row" key={r.enrollmentNo}><span>{i+1}</span><p>{r.name}</p><strong>{r.combined}</strong></div>)}</section><section className="analysis-panel academic-rank-panel"><h3>Bottom 5 At-Risk Students</h3>{[...combinedRankRows].reverse().slice(0,5).map((r,i)=><div className="academic-rank-row" key={r.enrollmentNo}><span>{i+1}</span><p>{r.name}</p><strong className="is-negative">{r.combined}</strong></div>)}</section></div></div></section></>}
      {data && view === "summary" && <><section className="summary-heading"><div><h2>Academic Summary</h2><p>Compare Midsem 1 and Midsem 2 with the same ClassPulse academic analysis theme.</p></div></section><section className="summary-metric-grid"><Metric label="Overall Class Average" value={summaryAverage} detail="combined average"/><Metric label="Highest Combined Score" value={combinedHighest} detail={combinedHighestNames[0]||"—"}/><Metric label="Overall Pass Rate" value={`${summaryPassRate}%`} detail="students scoring 12 or more"/><Metric label="Midsem 1 Average" value={midsem1.average} detail="out of 30 marks"/><Metric label="Midsem 2 Average" value={midsem2.average} detail="out of 30 marks"/></section><section className="summary-main-grid"><section className="analysis-panel summary-student-panel"><div className="summary-table-toolbar"><div className="analysis-panel-head"><div><h3>Filtered Students</h3><p style={{marginTop:4,color:"#98a2b3",fontSize:11}}>Showing {summaryRows.length} of {students.length} students</p></div><span className="analysis-count">{summaryRows.length} Students</span></div><div className="summary-inline-controls"><label><span>Exam</span><select value={summaryExamDraft} onChange={e=>setSummaryExamDraft(e.target.value as SummaryExam)}><option value="combined">Combined</option><option value="midsem1">Midsem 1</option><option value="midsem2">Midsem 2</option><option value="max">Max</option></select></label><label><span>Performance Tier</span><select value={summaryTierDraft} onChange={e=>setSummaryTierDraft(e.target.value as Tier|"all")}><option value="all">All Tiers</option>{TIERS.map(t=><option key={t} value={t}>{t}</option>)}</select></label><label><span>Lower Bound</span><input type="number" min="0" max="30" value={summaryLowerDraft} onChange={e=>setSummaryLowerDraft(e.target.value)}/></label><label><span>Upper Bound</span><input type="number" min="0" max="30" value={summaryUpperDraft} onChange={e=>setSummaryUpperDraft(e.target.value)}/></label><label><span>Sort</span><select value={summarySortDraft} onChange={e=>setSummarySortDraft(e.target.value as SummarySort)}><option value="none">No Sort</option><option value="desc">High to Low</option><option value="asc">Low to High</option></select></label><button className="analysis-primary summary-apply-button" onClick={applySummaryControls}>Apply</button><button className="analysis-secondary summary-reset-button" onClick={resetSummaryControls}>Reset</button></div></div><div className="analysis-table-wrap summary-table-wrap"><table className="analysis-table"><thead><tr><th>Enrollment</th><th>Student</th><th>Marks</th><th>Tier</th></tr></thead><tbody>{summaryRows.map(row=><tr key={row.enrollmentNo}><td>{row.enrollmentNo}</td><td><span className="student-cell"><span className="student-avatar">{initials(row.name)}</span>{row.name}</span></td><td className={`tier-mark ${gradeClass(row.tier)}`}><strong>{row.marks}</strong></td><td><span className={`analysis-grade-badge ${gradeClass(row.tier)}`}>{row.tier}</span></td></tr>)}{!summaryRows.length&&<tr><td colSpan={4} style={{textAlign:"center",padding:32,color:"#667085"}}>No students match these filters.</td></tr>}</tbody></table></div></section><div className="summary-right-column"><section className="analysis-panel summary-rank-panel"><div className="summary-rank-title"><div><h3>Marks Increase (Top 5)</h3><p>Students whose Midsem 2 score improved.</p></div></div>{increases.length?increases.map((row,i)=><div className="summary-rank-row" key={row.enrollmentNo}><span>{i+1}.</span><p>{row.name}</p><strong className="change-up">+{row.change}</strong></div>):<p className="summary-empty">No increases.</p>}</section><section className="analysis-panel summary-rank-panel"><div className="summary-rank-title"><div><h3>Marks Decrease (Top 5)</h3><p>Students whose Midsem 2 score fell.</p></div></div>{decreases.length?decreases.map((row,i)=><div className="summary-rank-row" key={row.enrollmentNo}><span>{i+1}.</span><p>{row.name}</p><strong className="change-down">{row.change}</strong></div>):<p className="summary-empty">No decreases.</p>}</section></div></section></section></>}
    </main>
    <style jsx global>{`
      ...
      .academic-sno-col { width: 5%; }
      .academic-enrollment-col { width: 21%; }
      .academic-name-col { width: 34%; }
      ...
      .combined-panel-title h3 { margin:0; }.combined-panel-title p { margin:5px 0 0; color:#98a2b3; font-size:11px; }.combined-panel-title>span { color:#98a2b3; font-size:11px; white-space:nowrap; }.combined-panel-title>span strong { color:#344054; }.combined-chart { height:300px; margin-top:10px; }.combined-tier-panel { padding:18px; }
      .tier-mark { font-weight:700; }.tier-mark.excellent { color:#2563eb!important; }.tier-mark.good { color:#15966a!important; }.tier-mark.attention { color:#f59e0b!important; }.tier-mark.risk { color:#ef4444!important; }
      .summary-heading { margin:4px 0 18px; }.summary-heading h2 { margin:0; font-size:22px; }.summary-heading p { margin:6px 0 0; color:#667085; font-size:13px; }.summary-metric-grid { display:grid; grid-template-columns:repeat(5,minmax(0,1fr)); gap:12px; margin-bottom:16px; }.summary-main-grid { display:grid; grid-template-columns:minmax(0,1.35fr) minmax(340px,.9fr); gap:16px; align-items:start; }.summary-student-panel { min-width:0; }.summary-table-wrap { max-height:650px; min-height:650px; overflow:auto; }.summary-table-wrap .analysis-table { table-layout:fixed; }.summary-table-wrap .analysis-table th,.summary-table-wrap .analysis-table td { white-space:nowrap; }.summary-table-wrap .analysis-table th:nth-child(1),.summary-table-wrap .analysis-table td:nth-child(1) { width:24%; }.summary-table-wrap .analysis-table th:nth-child(2),.summary-table-wrap .analysis-table td:nth-child(2) { width:40%; }.summary-table-wrap .analysis-table th:nth-child(3),.summary-table-wrap .analysis-table td:nth-child(3) { width:16%; text-align:center; }.summary-table-wrap .analysis-table th:nth-child(4),.summary-table-wrap .analysis-table td:nth-child(4) { width:20%; text-align:center; }.summary-right-column { display:grid; gap:16px; }.summary-rank-panel { padding:18px; }.summary-rank-title h3 { margin:0; }.summary-rank-title p { margin:5px 0 12px; color:#98a2b3; font-size:11px; }.summary-rank-row { display:grid; grid-template-columns:24px minmax(0,1fr) auto; align-items:center; gap:8px; padding:8px 0; border-bottom:1px solid #eef1f5; font-size:12px; }.summary-rank-row:last-child { border-bottom:0; }.summary-rank-row>span { color:#667085; }.summary-rank-row p { margin:0; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; color:#344054; }.summary-rank-row strong { font-size:12px; }.summary-empty { color:#98a2b3; font-size:12px; margin:10px 0 0; }
      @media (max-width:1200px) { .combined-layout,.summary-main-grid { grid-template-columns:1fr; }.combined-right-column { grid-template-columns:1fr 1fr; }.combined-right-column .academic-rank-grid { grid-column:1/-1; }.summary-metric-grid { grid-template-columns:repeat(3,minmax(0,1fr)); }.combined-inline-controls { grid-template-columns:1fr 1fr auto; }.summary-inline-controls { grid-template-columns:repeat(4,minmax(0,1fr)); } }
      @media (max-width:800px) { .combined-right-column,.summary-metric-grid { grid-template-columns:1fr; }.combined-inline-controls,.summary-inline-controls { grid-template-columns:1fr; }.combined-table-wrap,.summary-table-wrap { min-height:480px; max-height:480px; }.academic-student-table { min-width:760px; } }
    `}</style>
  </div>;
}
