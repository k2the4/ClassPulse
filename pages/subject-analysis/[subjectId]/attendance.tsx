...
export default function SubjectAttendancePage() {
  ...
  useEffect(() => {
    ...
    try {
      const raw = window.sessionStorage.getItem(`classpulse-attendance-trend:${subjectId}`);
    ...
    setPreviousMonth(savedPrevious); setCurrentMonth(savedCurrent); setTrendCriteria(savedCriteria);
    loadAnalysis(false, savedPrevious, savedCurrent, savedCriteria);
  ...
  return <div className="analysis-layout">
    <aside className="analysis-sidebar"><div className="analysis-brand"><span className="analysis-brand__mark"><BarChart3 size={18} /></span><span>ClassPulse</span></div><nav className="analysis-side-nav"><a href="/dashboard"><LayoutDashboard size={18} />Dashboard</a><a href={typeof subjectId === "string" ? `/subject-analysis/${subjectId}/attendance` : "#"} className="is-active"><BookOpen size={18} />Class Analysis</a><a href="/subject-analysis"><GraduationCap size={18} />Subject Analysis</a></nav><div className="analysis-side-footer">ClassPulse Teacher Portal</div></aside>
    <main className="analysis-page">
      ...
      {data && view === "trend" && <>
        <section className="analysis-hero"><div className="analysis-hero-copy"><h2>Attendance Trend</h2><p>Compare attendance between two months and identify increasing, decreasing, and stable students.</p></div><Metric icon={<Users size={19} />} label={`Class Average (${previousMonth || "Previous"})`} value={`${previousAverage}%`} /><Metric icon={<Gauge size={19} />} label={`Class Average (${currentMonth || "Current"})`} value={`${currentAverage}%`} change={averageChange} /><Metric icon={<Sparkles size={19} />} label="Highest Attendance" value={highestAttendanceStudent?.name || "—"} detail={highestAttendanceStudent ? `${round1(highestAttendanceStudent.attendancePct.currMonth)}%` : "No students"} /></section>
        <section className="analysis-panel analysis-settings"><h3>Trend Comparison Settings</h3><div className="analysis-settings-grid"><div><label>First Month</label><select value={previousMonth} onChange={(e) => setPreviousMonth(e.target.value)}>{availableMonths.map((month) => <option key={month}>{month}</option>)}</select></div><div><label>Second Month</label><select value={currentMonth} onChange={(e) => setCurrentMonth(e.target.value)}>{availableMonths.map((month) => <option key={month}>{month}</option>)}</select></div><div><label>Trend Criteria (%)</label><input type="number" min="0" max="100" value={trendCriteria} onChange={(e) => setTrendCriteria(Number(e.target.value))} /></div><button className="analysis-primary" onClick={() => { try { window.sessionStorage.setItem(`classpulse-attendance-trend:${subjectId}`, JSON.stringify({ previousMonth, currentMonth, trendCriteria })); } catch {} loadAnalysis(false, previousMonth, currentMonth, trendCriteria); }} disabled={loading || !previousMonth || !currentMonth || previousMonth === currentMonth}><SlidersHorizontal size={15} />{loading ? "Applying..." : "Apply Comparison"}</button></div><div className="analysis-note"><AlertCircle size={14} />Trend calculation: a change of <strong>±{trendCriteria} percentage points</strong> or more is Increasing or Decreasing.</div></section>
