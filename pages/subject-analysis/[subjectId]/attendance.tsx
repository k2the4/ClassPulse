...
export default function SubjectAttendancePage() {
  ...
  async function loadAnalysis(sync = false, previous = previousMonth, current = currentMonth, criteria = trendCriteria) {
    ...
    try {
      ...
      const response = await fetch(`/api/analysis/subject/${subjectId}?${params.toString()}`);
  ...
  const visibleStudents = filteredStudents.slice((page - 1) * perPage, page * perPage);
  ...
  function openRisk(options: { trend?: string; lower?: number; upper?: number }) {
    setRiskTrendFilter(options.trend || "All"); setLowerBound(options.lower ?? 0); setUpperBound(options.upper ?? 100); setView("risk");
  }
  ...
  return <div className="analysis-layout">
    <aside className="analysis-sidebar">
      ...
      <nav className="analysis-side-nav">
        ...
        <a className="is-active" href={typeof subjectId === "string" ? `/subject-analysis/${subjectId}/attendance` : "#"}><GraduationCap size={18} />Subject Analysis</a>
      </nav>
    ...
    <main className="analysis-page">
      <header className="analysis-topbar">
        ...
        <div className="analysis-top-actions"><RawDataButton sheetId={sheetId} /><button className="analysis-primary" onClick={() => loadAnalysis(true)} disabled={syncing}><RefreshCw size={15} className={syncing ? "animate-spin" : ""} />{syncing ? "Syncing..." : "Sync now"}</button></div>
      </header>
...
function Metric({ icon, label, value, change, detail, good }: { icon: ReactNode; label: string; value: string | number; change?: number; detail?: string; good?: boolean }) {
  return <div className="analysis-metric"><div className="analysis-metric-icon">{icon}</div><div><span>{label}</span><strong>{value}</strong>{change !== undefined && <small className={change > 0 || good ? "change-up" : change < 0 ? "change-down" : ""}>{change > 0 ? "↑ +" : change < 0 ? "↓ " : ""}{change}%</small>}{detail && <small>{detail}</small>}</div></div>;
}
