...
interface SubjectOption { id: string; classId: string; label: string; }
    ...
    <div className="min-h-screen">
      <header className="border-b border-gray-100 bg-white">
        <div className="max-w-[1700px] mx-auto px-8 py-4 flex items-center justify-between">
          <div><h1 className="font-semibold text-gray-900">ClassPulse</h1><p className="text-sm text-gray-500">Welcome, {teacherName}</p></div>
          <button onClick={() => signOut({ callbackUrl: "/login" })} className="text-sm text-gray-500 hover:text-gray-900">Sign out</button>
        </div>
      </header>
      <main className="max-w-[1700px] mx-auto px-8 py-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <Link href="/class-analysis" className="bg-white rounded-2xl border border-gray-100 p-6 hover:bg-gray-50">
          ...
          <Link href="/subject-analysis" className="bg-white rounded-2xl border border-gray-100 p-6 hover:bg-gray-50">
...
export const getServerSideProps: GetServerSideProps = async (ctx) => {
  ...
  for (const section of sections) classMap.set(section.class.id, { id: section.class.id, label: formatClassLabel(section.class.department.name, section.name, section.class.semester) });
  ...
  return { props: {
    ...
    subjects: subjectsRaw.map((subject) => ({ id: subject.id, classId: subject.section.classId, label: `${subject.name} (${subject.code})` })),
  } };
};
function formatClassLabel(department: string, classNumber: string, semester: number) { return `${department}${classNumber} Sem ${semester}`; }
