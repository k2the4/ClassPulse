      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-5 space-y-2">
        {classes.map((classOption) => (
          <Link
            ...
            className="block rounded-xl border border-gray-100 px-4 py-4 text-sm text-gray-900 hover:bg-gray-50"
          >
            {classOption.label}
          </Link>
        ))}
        {!classes.length && (
          <p className="text-sm text-gray-400 py-4">No classes are assigned to you.</p>
        )}
      </div>
    </div>
  );
