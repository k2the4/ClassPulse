...
export default function AcademicPage() {
  ...
  const subjectNames = (data as any)?.subjectNames || {};
  ...
  const summaryRankedRows = useMemo(() => [...summaryRows].filter((row) => Number.isFinite(Number(row.rowValue))).sort((a, b) => b.rowValue - a.rowValue), [summaryRows]);
  ...
  const applySummaryFilters = () => { setSummaryExam(summaryExamDraft); setSummaryTier(summaryTierDraft); setSummaryLower(summaryLowerDraft); setSummaryUpper(summaryUpperDraft); setSummarySort(summarySortDraft); };
  const resetSummaryFilters = () => { setSummaryExamDraft("combined"); setSummaryTierDraft("all"); setSummaryLowerDraft("0"); setSummaryUpperDraft("12"); setSummarySortDraft("none"); setSummaryExam("combined"); setSummaryTier("all"); setSummaryLower("0"); setSummaryUpper("12"); setSummarySort("none"); };
  ...
  return <div className="analysis-layout">
    ...
    <main className="analysis-main">
      ...
      {data && view === "summary" && <>
        ...
        <section className="summary-content-grid"><section className="analysis-panel summary-table-panel"><div className="analysis-panel-head"><div><h3>Filtered Students</h3><p style={{ marginTop: 4, color: "#98a2b3", fontSize: 11 }}>Showing {filteredSummaryRows.length} of {students.length} students</p></div><span className="analysis-count">{filteredSummaryRows.length} Students</span></div><div className="summary-table-scroll"><table className="summary-data-table"><colgroup><col className="summary-col-sno" /><col className="summary-col-enrollment" /><col className="summary-col-student" />{summarySubjectCodes.map((code) => <col key={code} className="summary-col-subject" />)}<col className="summary-col-total" /><col className="summary-col-tier" /></colgroup><thead><tr><th scope="col">{summaryShowRank ? "Rank" : "S.No."}</th><th scope="col">Enrollment No.</th><th scope="col">Student</th>{summarySubjectCodes.map((code) => <th key={code} scope="col" title={subjectNames[code] || code}>{code}</th>)}<th scope="col">Total</th><th scope="col">Tier</th></tr></thead><tbody>{filteredSummaryRows.map((row, index) => <tr key={row.enrollmentNo}><td>{index + 1}</td><td>{row.enrollmentNo}</td><td className="summary-student-name">{row.name}</td>{summarySubjectCodes.map((code) => { const subject = row.subjects.find((s: any) => s.code === code); const value = subject?.value; const lower = Number(summaryLower); const upper = Number(summaryUpper); const inRange = value != null && value >= (Number.isFinite(lower) ? lower : 0) && value <= (Number.isFinite(upper) ? upper : 30); return <td key={code} className={inRange ? value < 12 ? "summary-score summary-score-low" : "summary-score summary-score-pass" : "summary-score summary-score-empty"}>{inRange ? value : ""}</td>; })}<td className="summary-total">{row.total}</td><td className="summary-tier" style={{ color: TIER_COLORS[row.tier] }}>{row.tier}</td></tr>)}{!filteredSummaryRows.length && <tr><td colSpan={summarySubjectCodes.length + 5} className="summary-empty-state">No students match the selected criteria.</td></tr>}</tbody></table></div></section><div className="summary-right-stack"><section className="analysis-panel summary-rank-panel"><h3>Top 5 Students</h3><p>Highest scores for the selected examination.</p><div>{summaryRankedRows.slice(0, 5).map((row, index) => <div key={`top-${row.enrollmentNo}`} className="summary-rank-row is-top"><span>{index + 1}</span><span>{row.name}</span><span>{row.rowValue}</span></div>)}</div></section><section className="analysis-panel summary-rank-panel"><h3>Bottom 5 Students</h3><p>Lowest scores for the selected examination.</p><div>{summaryRankedRows.slice(-5).reverse().map((row, index) => <div key={`bottom-${row.enrollmentNo}`} className="summary-rank-row is-bottom"><span>{index + 1}</span><span>{row.name}</span><span>{row.rowValue}</span></div>)}</div></section></div></section>
      </>}
    </main>
  </div>;
}
