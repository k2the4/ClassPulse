  ...
  return <div className="analysis-layout">
    ...
    <main className="analysis-main">
      ...
      {data && view === "summary" && <>
        ...
        <section className="summary-content-grid"><section className="analysis-panel summary-table-panel" style={{ padding: 18, minWidth: 0 }}><div className="analysis-panel-head" style={{ marginBottom: 14 }}><div><h3>Filtered Students</h3><p style={{ marginTop: 4, color: "#98a2b3", fontSize: 11 }}>Showing {filteredSummaryRows.length} of {students.length} students</p></div><span className="analysis-count">{filteredSummaryRows.length} Students</span></div><div className="summary-table-wrap" style={{ width: "100%", overflowX: "auto", overflowY: "auto" }}><table className="analysis-table" style={{ width: "100%", minWidth: 760, tableLayout: "fixed", borderCollapse: "collapse" }}><colgroup><col style={{ width: "6%" }} /><col style={{ width: "18%" }} /><col style={{ width: "25%" }} />{summarySubjectCodes.map((code) => <col key={code} style={{ width: `${summarySubjectCodes.length ? 31.5 / summarySubjectCodes.length : 0}%` }} />)}<col style={{ width: "8%" }} /><col style={{ width: "11.5%" }} /></colgroup><thead><tr><th style={{ textAlign: "center", padding: "11px 8px" }}>{summaryShowRank ? "Rank" : "S.No."}</th><th style={{ textAlign: "left", padding: "11px 10px" }}>Enrollment No.</th><th style={{ textAlign: "left", padding: "11px 10px" }}>Student</th>{summarySubjectCodes.map((code) => <th key={code} style={{ textAlign: "center", padding: "11px 6px", whiteSpace: "nowrap" }}>{code}</th>)}<th style={{ textAlign: "center", padding: "11px 8px", whiteSpace: "nowrap" }}>Total</th><th style={{ textAlign: "center", padding: "11px 8px", whiteSpace: "nowrap" }}>Tier</th></tr></thead><tbody>{filteredSummaryRows.map((row, index) => <tr key={row.enrollmentNo}><td style={{ textAlign: "center", padding: "10px 8px" }}>{index + 1}</td><td style={{ textAlign: "left", padding: "10px" }}>{row.enrollmentNo}</td><td style={{ textAlign: "left", padding: "10px", fontWeight: 600 }}>{row.name}</td>{summarySubjectCodes.map((code) => { const subject = row.subjects.find((s: any) => s.code === code); const value = subject?.value; const lower = Number(summaryLower); const upper = Number(summaryUpper); const inRange = value != null && value >= (Number.isFinite(lower) ? lower : 0) && value <= (Number.isFinite(upper) ? upper : 30); return <td key={code} style={{ textAlign: "center", padding: "10px 6px", color: inRange && value < 12 ? "#ef4444" : inRange ? "#15966a" : "#98a2b3", fontWeight: inRange ? 600 : 400 }}>{inRange ? value : ""}</td>; })}<td style={{ textAlign: "center", padding: "10px 8px", fontWeight: 700 }}>{row.total}</td><td style={{ textAlign: "center", padding: "10px 8px", fontWeight: 700, color: TIER_COLORS[row.tier] }}>{row.tier}</td></tr>)}{!filteredSummaryRows.length && <tr><td colSpan={summarySubjectCodes.length + 5} style={{ textAlign: "center", padding: 32, color: "#667085" }}>No students match the selected criteria.</td></tr>}</tbody></table></div></section></section>
      </>}
    </main>
  </div>;
}
