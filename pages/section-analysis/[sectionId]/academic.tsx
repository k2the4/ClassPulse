...
function Metric({ label, value, detail }: { label: string; value: string | number; detail: string }) { return <div className="rounded-2xl border border-[#e6e5e2] bg-white px-5 py-4 shadow-[0_8px_28px_rgba(31,35,49,0.04)]"><div className="text-[11px] font-semibold text-[#6f7890]">{label}</div><div className="mt-1 text-[25px] font-extrabold tracking-[-1px] text-[#17223b]">{value}</div><div className="mt-1 text-[10px] text-[#98a2b3]">{detail}</div></div>; }
...
export default function AcademicPage() {
  ...
  async function loadAnalysis(sync = false) {
    ...
    try { const res = await fetch(`/api/analysis/section/${sectionId}${sync ? "?sync=1" : ""}`); const json = await res.json(); if (!res.ok) throw new Error(json.detail ? `${json.error}: ${json.detail}` : json.error || "Failed to load academic analysis"); setData(json.data); setComputedAt(json.computedAt || ""); setSheetId(json.sheetId || null); }
  ...
  const activeStats = useMemo(() => {
    const rows = students.map((student: any) => {
      if (view === "combined") {
        ...
        else if (scoreBasis === "max") { const midsem1 = student.examMarks?.midsem1; const midsem2 = student.examMarks?.midsem2; const available = [midsem1, midsem2].filter((value: any): value is number => value !== null && value !== undefined); marks = available.length ? Math.max(...available) : null; max = Math.max(student.examMarks?.midsem1Max || 0, student.examMarks?.midsem2Max || 0); }
      ...
      const marks = student.examMarks?.[exam] ?? null; const max = student.examMarks?.[`${exam}Max`] || 0; const subjects = student.examMarks?.[`${exam}Subjects`] || [];
      return { enrollmentNo: student.enrollmentNo, name: student.name, marks, max, subjects };
    }).filter((row: any) => row.marks !== null);
    const marks = rows.map((row: any) => Number(row.marks)); const max = rows[0]?.max || 0;
  ...
  return <div className="analysis-layout min-h-screen bg-[#fffdf8] text-[#17223b]">
    ...
    <main className="analysis-page">
      ...
      {data && <>
        <section className="analysis-hero"><div className="analysis-hero-copy"><h2>{activeLabel}</h2><p>{view === "combined" ? "Combined Midsem 1 and Midsem 2 performance across all subjects." : "Marks across all subjects, class statistics, and performance tiers."}</p></div><Metric label="Class Average" value={`${activeStats.average}/${activeStats.max || 0}`} detail="class average across subjects" /><Metric label="Class Median" value={`${activeStats.median}/${activeStats.max || 0}`} detail="middle class score" /><Metric label="Pass Rate" value={`${activeStats.passRate}%`} detail="students at or above 40%" /></section>
        <section className="analysis-content-grid academic-content-grid">
          <section className="analysis-panel analysis-table-panel">
            ...
            <div className="max-h-[500px] overflow-y-auto overflow-x-hidden"><table className="w-full min-w-0 table-fixed border-collapse text-[11px]"><colgroup>{showRank && <col className="w-[40px]" />}<col className="w-[160px]" />{activeStats.subjectCodes.map((code: string) => <col key={code} className="w-[48px]" />)}<col className="w-[55px]" /><col className="w-[55px]" /><col className="w-[92px]" /></colgroup><thead><tr className="border-b border-[#edf0f4] text-left text-[10px] font-bold uppercase tracking-[0.2px] text-[#7b8498]">{showRank && <th className="sticky left-0 top-0 z-20 bg-white px-2 py-3 text-center">Rank</th>}<th className={`sticky ${showRank ? "left-[40px]" : "left-0"} top-0 z-20 bg-white px-2 py-3`}>Student Name</th>{activeStats.subjectCodes.map((code: string) => <th key={code} className="sticky top-0 z-10 whitespace-nowrap bg-white px-1.5 py-3">{code}</th>)}<th className="sticky top-0 z-10 whitespace-nowrap bg-white px-1.5 py-3">Total</th><th className="sticky top-0 z-10 whitespace-nowrap bg-white px-1.5 py-3">%age</th><th className="sticky top-0 z-10 whitespace-nowrap bg-white px-1.5 py-3">Grade</th></tr></thead><tbody>
              {displayedRows.map((row: any, index: number) => { const rowMax = Number(row.max) || activeStats.max; const pct = rowMax > 0 ? Math.round(Number(row.marks) / rowMax * 100) : 0; const tier = tierFor(Number(row.marks), rowMax); const grade = gradeFor(Number(row.marks), rowMax); return <tr key={row.enrollmentNo} className="border-b border-[#f0f1f3] last:border-0">{showRank && <td className="sticky left-0 z-10 bg-white px-2 py-3 text-center font-semibold text-[#626b80]">{index + 1}</td>}<td className={`sticky ${showRank ? "left-[40px]" : "left-0"} z-10 bg-white px-2 py-3 font-semibold text-[#17223b]`}><div className="flex items-center gap-2"><span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-[#eeebff] text-[9px] font-extrabold text-[#5b4ee6]">{initials(row.name)}</span><span className="truncate">{row.name}</span></div></td>{activeStats.subjectCodes.map((code: string) => { const subject = (row.subjects || []).find((item: any) => item.code === code); return <td key={code} className={`px-1.5 py-3 ${subject?.marks === null || subject?.marks === undefined ? "text-[#98a2b3]" : subject.pass === false ? "font-semibold text-[#ef4444]" : "text-[#15966a]"}`}>{subject?.marks ?? "—"}</td>; })}<td className="px-1.5 py-3 font-bold text-[#17223b]">{row.marks}</td><td className="px-1.5 py-3 text-[#626b80]">{pct}%</td><td className="px-1.5 py-3"><button onClick={() => setSelectedTier(tier)} className="inline-flex max-w-full whitespace-nowrap rounded-full px-2 py-1 text-[9px] font-bold" style={{ backgroundColor: `${TIER_COLORS[grade]}18`, color: TIER_COLORS[grade] }}>{grade}</button></td></tr>; })}
            </tbody></table></div>
          </section>
          <div className="space-y-5"><div className="grid grid-cols-2 gap-4"><Metric label="Class Average" value={activeStats.average} detail={`out of ${activeStats.max || 0}`} /><Metric label="Class Median" value={activeStats.median} detail="middle class score" /><Metric label="Highest Score" value={activeStats.highest} detail={highestNames.length ? highestNames[0] : "top score"} /><Metric label="Pass Rate" value={`${activeStats.passRate}%`} detail={`${activeStats.rows.length} students assessed`} /></div>
