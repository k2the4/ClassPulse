      ...
      {data && <>
        ...
        <section className="analysis-content-grid academic-content-grid">
          <section className="analysis-panel analysis-table-panel">
            ...
            <div className="overflow-auto"><table className="w-full min-w-[820px] border-collapse text-[12px]"><thead><tr className="border-b border-[#edf0f4] text-left text-[10px] font-bold uppercase tracking-[0.2px] text-[#7b8498]"><th className="sticky left-0 z-10 bg-white px-3 py-3">Student Name</th>{activeStats.subjectCodes.map((code: string) => <th key={code} className="whitespace-nowrap px-3 py-3">{code}</th>)}<th className="whitespace-nowrap px-3 py-3">Total</th>{view === "combined" && <th className="whitespace-nowrap px-3 py-3">Grade</th>}{view !== "combined" && <><th className="whitespace-nowrap px-3 py-3">%age</th><th className="whitespace-nowrap px-3 py-3">Tier</th></>}</tr></thead><tbody>
              {displayedRows.map((row: any) => { const rowMax = Number(row.max) || activeStats.max; const pct = rowMax > 0 ? Math.round(Number(row.marks) / rowMax * 100) : 0; const tier = tierFor(Number(row.marks), rowMax); const grade = gradeFor(Number(row.marks), rowMax); return <tr key={row.enrollmentNo} className="border-b border-[#f0f1f3] last:border-0"><td className="sticky left-0 z-10 bg-white px-3 py-3 font-semibold text-[#17223b]"><div className="flex items-center gap-2"><span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-[#eeebff] text-[9px] font-extrabold text-[#5b4ee6]">{initials(row.name)}</span><span className="whitespace-nowrap">{row.name}</span></div></td>{activeStats.subjectCodes.map((code: string) => { const subject = (row.subjects || []).find((item: any) => item.code === code); return <td key={code} className={`px-3 py-3 ${subject?.marks === null || subject?.marks === undefined ? "text-[#98a2b3]" : subject.pass === false ? "font-semibold text-[#ef4444]" : "text-[#15966a]"}`}>{subject?.marks ?? "—"}</td>; })}<td className="px-3 py-3 font-bold text-[#17223b]">{row.marks}</td>{view === "combined" ? <td className="px-3 py-3"><button onClick={() => setSelectedTier(tier)} className="rounded-full px-2.5 py-1 text-[9px] font-bold" style={{ backgroundColor: `${TIER_COLORS[tier]}18`, color: TIER_COLORS[tier] }}>{grade}</button></td> : <><td className="px-3 py-3 text-[#626b80]">{pct}%</td><td className="px-3 py-3"><button onClick={() => setSelectedTier(tier)} className="rounded-full px-2.5 py-1 text-[9px] font-bold" style={{ backgroundColor: `${TIER_COLORS[tier]}18`, color: TIER_COLORS[tier] }}>{tier}</button></td></>}</tr>; })}
            </tbody></table></div>
          </section>
