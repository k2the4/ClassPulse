      ...
      {data && <>
        ...
        <section className="analysis-content-grid academic-content-grid">
          <section className="analysis-panel analysis-table-panel">
            ...
            <div className="max-h-[500px] overflow-y-auto overflow-x-hidden"><table className="w-full min-w-0 table-fixed border-collapse text-[12px]"><colgroup><col className="w-[185px]" />{activeStats.subjectCodes.map((code: string) => <col key={code} className="w-[58px]" />)}<col className="w-[62px]" /><col className="w-[62px]" /><col className="w-[104px]" /></colgroup><thead><tr className="border-b border-[#edf0f4] text-left text-[10px] font-bold uppercase tracking-[0.2px] text-[#7b8498]"><th className="sticky left-0 top-0 z-20 bg-white px-3 py-3">Student Name</th>{activeStats.subjectCodes.map((code: string) => <th key={code} className="sticky top-0 z-10 whitespace-nowrap bg-white px-2 py-3">{code}</th>)}<th className="sticky top-0 z-10 whitespace-nowrap bg-white px-2 py-3">Total</th><th className="sticky top-0 z-10 whitespace-nowrap bg-white px-2 py-3">%age</th><th className="sticky top-0 z-10 whitespace-nowrap bg-white px-2 py-3">Grade</th></tr></thead><tbody>
              {displayedRows.map((row: any) => { const rowMax = Number(row.max) || activeStats.max; const pct = rowMax > 0 ? Math.round(Number(row.marks) / rowMax * 100) : 0; const tier = tierFor(Number(row.marks), rowMax); const grade = gradeFor(Number(row.marks), rowMax); return <tr key={row.enrollmentNo} className="border-b border-[#f0f1f3] last:border-0"><td className="sticky left-0 z-10 bg-white px-3 py-3 font-semibold text-[#17223b]"><div className="flex items-center gap-2"><span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-[#eeebff] text-[9px] font-extrabold text-[#5b4ee6]">{initials(row.name)}</span><span className="truncate">{row.name}</span></div></td>{activeStats.subjectCodes.map((code: string) => { const subject = (row.subjects || []).find((item: any) => item.code === code); return <td key={code} className={`px-2 py-3 ${subject?.marks === null || subject?.marks === undefined ? "text-[#98a2b3]" : subject.pass === false ? "font-semibold text-[#ef4444]" : "text-[#15966a]"}`}>{subject?.marks ?? "—"}</td>; })}<td className="px-2 py-3 font-bold text-[#17223b]">{row.marks}</td><td className="px-2 py-3 text-[#626b80]">{pct}%</td><td className="px-2 py-3"><button onClick={() => setSelectedTier(tier)} className="whitespace-nowrap rounded-full px-2 py-1 text-[9px] font-bold" style={{ backgroundColor: `${TIER_COLORS[grade]}18`, color: TIER_COLORS[grade] }}>{grade}</button></td></tr>; })}
            </tbody></table></div>
          </section>
